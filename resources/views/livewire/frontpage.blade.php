<div class="divide-y divide-gray-800" x-data="{ show: false }">
    <nav class="flex items-center bg-gray-800 px-8 py-2 shadow-lg">
        <div>
            ...
            <ul class="hidden sm:flex sm:text-left text-gray-200 text-xs">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <a href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();">
                        <li class="cursor-pointer px-4 py-2 hover:bg-gray-800">{{ __('Log out') }}</li>
                    </a>
                </form>
    ...
    <div class="sm:flex sm:min-h-screen">
        <aside class="bg-gray-800 text-gray-700 divide-y divide-gray-700 divide-dashed sm:w-4/12 md:w-3/12 lg:w-2/12">
            {{-- Desktop Web View --}}
            <ul class="hidden text-gray-200 text-s sm:block sm:text-left">
                @foreach ($sideBarLinks as $item)
                    <a href="{{ url('/'.$item->slug) }}">
                        <li class="cursor-pointer px-8 py-4 hover:bg-gray-800">{{ $item->label }}</li>
                    </a>
                @endforeach                
            </ul>
            ...
            <section class="divide-y text-gray-900">
                ...
                <article class="markdown">
                    <div class="mt-5 text-sm">
                        {!! nl2br(Markdown::parse(e($content))) !!}
                    </div>
                </article>
            </section>
        </main>
